...
if (!fs.existsSync(SKILLS_DIR)) {
    ...
    console.error(`[HINT]  Set the SKILLS_DIR environment variable to your skills folder, or create the .agents/skills directory.`);
    process.exit(1);
}
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
    return {
        tools: [
            {
                name: "list_skills",
                description: "List all available skills/rules in the knowledge base. Returns folder names.",
            ...
            },
            {
                ...
                description: "Fetch the full markdown content of a specific skill to learn how to do a task.",
                inputSchema: {
                    type: "object",
                    properties: {
                        skill_name: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (name === "list_skills") {
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Could not read skills directory: ${error}`);
        }
    }
    if (name === "fetch_skill_rule") {
        const skillName = args?.skill_name;
        if (!skillName || typeof skillName !== "string") {
            throw new McpError(ErrorCode.InvalidParams, "Invalid skill_name.");
        }
        ...
        if (!targetDir.startsWith(resolvedSkillsDir + path.sep)) {
            throw new McpError(ErrorCode.InvalidParams, "Invalid skill_name path traversal detected.");
        }
        try {
            try {
            ...
            throw new McpError(ErrorCode.InvalidParams, `Skill '${skillName}' not found or unreadable. Check that the folder exists in ${SKILLS_DIR} and contains a .md file.`);
        }
    }
