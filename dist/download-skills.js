...
async function installSkillWithRetry(name, githubUrl, retryCount = 0) {
    try {
...
async function downloadSkillsmpPage1() {
    ...
    try {
        ...
        // Filter valid items
        const validSkills = items.map((item) => {
        ...
        for (let i = 0; i < validSkills.length; i += BATCH_SIZE) {
            ...
            await Promise.all(batch.map((skill) => installSkillWithRetry(skill.name, skill.githubUrl)));
        }
